...
com.ind.pos <- intersect(unlist(pos.ind), com.ind)
com.ind.neg <- intersect(unlist(neg.ind), com.ind)
...
#reading in the PC score matrix
  ...
  col1 <- colnames(gene.data)[1:ncol(gene.data)]
  col2 <- rep(NA, ncol(gene.data))
  col2[-match(names(na.var.gene),col1)] <- colnames(gene.data)[1:(ncol(gene.data)-length (na.var.gene))]
...
for(i in 1:30000){
    ...
    total.pc.count <- length(unlist(gene.sig.asso.pcs[gene.row.novar,])) + length(unlist(meth.sig.asso.pcs[meth.row.nona,]))

    rows.clinical <- match(com.ind.pos, unlist(clinical.pos[,1]))
    ...
    #write to a file
    write.table(final, file = "/mnt/ceph/kark6289/TCGA_analysis/MRGN_InferTrio/pos_ER/model.trio.MRGN1.txt", sep = "\t", row.names = FALSE,
