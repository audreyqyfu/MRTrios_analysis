...
pc.gene = as.data.frame(fread("/mnt/ceph/kark6289/TCGA_analysis/PCs/logit_data_PCs/PCA.GeneExp.negER.txt"))
...
#returns other 3 cols that have var NA and have 185 NA values
t <- apply(is.na(meth.data), 2, sum)
na.var.meth <- which(t > 184)
...
gene.sig.asso.pcs <- readRDS("/mnt/ceph/kark6289/TCGA_analysis/PCs/logit_data_PCs/gene.negER.sig.asso.pcs.RData")
meth.sig.asso.pcs <- readRDS("/mnt/ceph/kark6289/TCGA_analysis/PCs/logit_data_PCs/meth.negER.sig.asso.pcs.RData")

#matching individuals between common positive individuals and respective datasets
ind.col.meth = match(com.ind.neg, colnames(meth))
ind.col.gene = match(com.ind.neg, colnames(gene))
ind.col.cna = match(com.ind.neg, colnames(cna))
...
for(i in 1:nrow(trios)){
  ...
  if(is.na(trios[i,3]) == FALSE & is.na(trios[i,4]) == FALSE) {
    ...
    final <- cbind(i, which.model)
    
    #write to a file
    write.table(final, file = "/mnt/ceph/kark6289/TCGA_analysis/MRGN_InferTrio/negER/model.trio.MRGN.txt", sep = "\t", row.names = FALSE,
